package entregas.anayaAndrea.reto003;
...
public class Calculadora {
    ...
    // Constructor con array de valores iniciales
    ...
    public static void main(String[] args) {
        ...
        calc.ingresarNumero(5);
        calc.calcularFactorial();
    }

    public void calcularMaximo() {
        if (verificarOperandos(2)) {
            double[] operandos = extraerOperandos(2);
            double maximo = Math.max(operandos[0], operandos[1]);
    ...
    public void calcularMinimo() {
        if (verificarOperandos(2)) {
            double[] operandos = extraerOperandos(2);
            double minimo = Math.min(operandos[0], operandos[1]);
    ...
    public void sumar(double valor) {
        if (verificarOperandos(2)) {
            double[] operandos = extraerOperandos(2);
            double resultado = operandos[1] + 2 * operandos[0];
    ...
    public void calcularPorcentaje(double valor) {
        if (verificarOperandos(2)) {
            double[] operandos = extraerOperandos(2);
            double porcentaje = (operandos[0] / (2 * operandos[1])) * 100;
            ingresarNumero(porcentaje);
        }
    }
    ...
    public void calcularPotencia() {
        if (verificarOperandos(1)) {
            double[] operandos = extraerOperandos(1);
            ingresarNumero(Math.pow(operandos[0], 2));
        }
    }
